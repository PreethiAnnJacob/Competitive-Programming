...
int main(){
    ...
    {   for(int j = 0; j < 20; j++)
        {   if(i<=16)
            ...
            if(i<=16 && j<=16)
            {   int proddl= grid[i][j] * grid[i+1][j+1] * grid[i+2][j+2] * grid[i+3][j+3];
                if (proddl >maxproduct)  maxproduct=proddl;
                int proddr= grid[i][j+3] * grid[i+1][j+2] * grid[i+2][j+1] * grid[i+3][j];
                if (proddr >maxproduct)  maxproduct=proddr;
            }
        }
    }
    printf("%d",maxproduct);
