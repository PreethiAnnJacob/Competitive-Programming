...
int main(){
    ...
    for(int a0 = 0; a0 < t; a0++){
        ...
        {   int num=5; //since we know the next prime number is 5
            ...
            while(count<n) 
            {   int prime=1;
                //Any prime number>3 can be expressed as 6k+1 or 6k-1(~6k+5)
             
                if (num%3==0) prime=0; //instead of "if (num%2==0 || num%3==0) prime=0;" 
                //since only prime numbers less than 5 is 2 and 3. We have skipped the chances of even numbers by incrementing num by 2    
                else
